...
bool FilePos::FileReadNextBuffer()
{
	...
	if ( m_nFileByteOffset < m_nFileByteLen )
	{
		...
		if ( m_nReadGatherStart != -1 )
		{
			...
			{
				m_strReadGatherMarkup += MCD_STRMID( str, m_nReadGatherStart, m_nReadBufferStart - m_nReadGatherStart );
			}
...
void CMarkup::x_SetDebugState()
{
	...
	if ( m_nNodeLength || m_nNodeOffset || (m_pFilePos && (!m_iPos) && (!m_iPosParent) && ! m_pFilePos->FileAtTop()) )
	{
		if ( ! m_nNodeLength )
